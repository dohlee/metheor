...
fn benchmark_parameter_variations(c: &mut Criterion) {
    ...
    for min_cpgs in &[2, 4, 8, 16] {
        group.bench_function(BenchmarkId::new("MHL_min_cpgs", min_cpgs), |b| {
            b.iter(|| {
                let (_output_file, output_path) = setup_output_file();
                let output_str = output_path.to_str().unwrap();
...
fn benchmark_methylation_patterns(c: &mut Criterion) {
    ...
    let datasets = vec![
        (
            "low_methylation",
            generate_low_methylation_dataset().expect("Failed to generate dataset"),
        ),
        (
            "high_discordance",
            generate_high_discordance_dataset().expect("Failed to generate dataset"),
        ),
        (
            "high_depth",
            generate_high_depth_dataset().expect("Failed to generate dataset"),
        ),
    ];

    for (name, data) in datasets {
        group.bench_function(BenchmarkId::new("PDR", name), |b| {
            b.iter(|| {
                let (_output_file, output_path) = setup_output_file();
                let output_str = output_path.to_str().unwrap();
